...
def processNzedbPre():
    ...
    for preCSV in downloadLinks:
        ...
        if lastFileFromDisk is None or int(processingFile['lastfile']) > lastFileFromDisk['lastfile']:
            ...
            cleanFile = pandas.read_csv('unformattedDL.gz', sep='\t', compression='gzip', header=None, na_values='\\N',
                                        usecols=[0, 8, 10, 14, 16, 18, 20, 22])
            ...
            try:
                ...
                ordering = ['name', 'filename', 'nuked', 'category', 'pretime', 'source', 'requestid', 'requestgroup',
                            'searchname']
